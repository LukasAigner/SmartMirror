...
Module.register("MMM-Carousel", {
	...
	notificationReceived: function (notification, payload, sender) {
		var position,
			positions = ["top_bar", "bottom_bar", "top_left", "bottom_left", "top_center", "bottom_center", "top_right", "bottom_right", "upper_third", "middle_center", "lower_third"];
				...
				if (this.slides === undefined && i === this.currentIndex) {
					...
					for (var s = 0; s < mods.length; s++) {
						if (typeof mods[s] === "string" && mods[s] === this[i].name) {
							...
							if (typeof mods[s].carouselId !== "undefined" && typeof this[i].data.config.carouselId !== "undefined" && mods[s].carouselId !== this[i].data.config.carouselId) {
								break;
							}
	...
	socketNotificationReceived: function (noti, payload) {
		...
		if (noti === "CAROUSEL_GOTO") {
			if (typeof payload === "number" || typeof payload === "string") {
				try {
					if (this.manualTransition(parseInt(payload) - 1)) this.restartTimer();
				} catch (err) {
					console.warn("Could not navigate to slide " + payload);
				}
			} else if (typeof payload === "object") {
